= `NonConformLoadGroup`
:toclevels: 4


+++Loads that do not follow a daily and seasonal load variation pattern.+++


[cols="h,3",width=65%]
|===
| Type
| Class

| URI
| `cim:NonConformLoadGroup`


| CIM standard (perspective)
| IEC61970 (Grid)



|===

== Class hierarchy
* xref::class/IdentifiedObject.adoc[`IdentifiedObject`]
** xref::class/LoadGroup.adoc[`LoadGroup`]
*** *`NonConformLoadGroup`*


== Slots



.Associated classes
[d2,svg,theme=4]
----
vars: {
  d2-config: {
    pad: 20
  }
}
NonConformLoadGroup {
  style {
    fill: "#eccfcb"
    shadow: true
  }
}



NonConformLoad {
  style {
    fill: "#eccfcb"
    shadow: true
  }
}



NonConformLoadSchedule {
  style {
    fill: "#eccfcb"
    shadow: true
  }
}



SubLoadArea {
  style {
    fill: "#eccfcb"
    shadow: true
  }
}




NonConformLoadGroup -> NonConformLoad: EnergyConsumers


NonConformLoadGroup -> NonConformLoadSchedule: NonConformLoadSchedules


NonConformLoadGroup -> SubLoadArea: SubLoadArea


legend: "" {
  style: {
    fill: transparent
    stroke: transparent
  }
  grid-rows: 3
  grid-columns: 2
  grid-gap: 10
  near: bottom-right
  iec61968_color: "" {
    style: {
      fill: "#d1e7c2"
      stroke: black
      stroke-width: 1
    }
    width: 10
    height: 10
  }
  iec61968_text: "IEC61968 (Enterprise)" {
    shape: text
  }
  iec61970_color: "" {
    style: {
      fill: "#eccfcb"
      stroke: black
      stroke-width: 1
    }
    width: 10
    height: 10
  }
  iec61970_text: "IEC61970 (Grid)" {
    shape: text
  }
  iec62325_color: "" {
    style: {
      fill: "#fffbef"
      stroke: black
      stroke-width: 1
    }
    width: 10
    height: 10
  }
  iec62325_text: "IEC62325 (Market)" {
    shape: text
  }
}
----


[cols="1,1,2,1",width=100%]
|===
| Name | Type | Description | Inherited from

| <<EnergyConsumers,`EnergyConsumers`>>
//| [[slots_table.EnergyConsumers]]<<EnergyConsumers,`EnergyConsumers`>>
| 1..* +
xref::class/NonConformLoad.adoc[`NonConformLoad`]
| +++Conform loads assigned to this ConformLoadGroup.+++
| _n/a_

| <<NonConformLoadSchedules,`NonConformLoadSchedules`>>
//| [[slots_table.NonConformLoadSchedules]]<<NonConformLoadSchedules,`NonConformLoadSchedules`>>
| 0..* +
xref::class/NonConformLoadSchedule.adoc[`NonConformLoadSchedule`]
| +++The NonConformLoadSchedules in the NonConformLoadGroup.+++
| _n/a_

| <<SubLoadArea,`SubLoadArea`>>
//| [[slots_table.SubLoadArea]]<<SubLoadArea,`SubLoadArea`>>
| 1 +
xref::class/SubLoadArea.adoc[`SubLoadArea`]
| +++The SubLoadArea where the Loadgroup belongs.+++
| xref::class/LoadGroup.adoc[`LoadGroup`]

| <<description,`description`>>
//| [[slots_table.description]]<<description,`description`>>
| 0..1 +
https://w3id.org/linkml/String[`string`]
| +++The description is a free human readable text describing or naming the object. It may be non unique and may not correlate to...+++
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]

| <<energyIdentCodeEic,`energyIdentCodeEic`>>
//| [[slots_table.energyIdentCodeEic]]<<energyIdentCodeEic,`energyIdentCodeEic`>>
| 0..1 +
https://w3id.org/linkml/String[`string`]
| +++The attribute is used for an exchange of the EIC code (Energy identification Code). The length of the string is 16...+++
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]

| <<mRID,`mRID`>>
//| [[slots_table.mRID]]<<mRID,`mRID`>>
| 1 +
https://w3id.org/linkml/String[`string`]
| +++Master resource identifier issued by a model authority. The mRID is unique within an exchange context. Global uniqueness is...+++
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]

| <<name,`name`>>
//| [[slots_table.name]]<<name,`name`>>
| 1 +
https://w3id.org/linkml/String[`string`]
| +++The name is any free human readable and possibly non unique text naming the object.+++
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]

| <<shortName,`shortName`>>
//| [[slots_table.shortName]]<<shortName,`shortName`>>
| 0..1 +
https://w3id.org/linkml/String[`string`]
| +++The attribute is used for an exchange of a human readable short name with length of the string 12 characters maximum.+++
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]
|===

'''


//[discrete]
[#description]
=== `description`
+++The description is a free human readable text describing or naming the object. It may be non unique and may not correlate to a naming hierarchy.+++


[cols="h,4",width=65%]
|===
| URI
| `cim:IdentifiedObject.description`
| Cardinality
| 0..1
| Type
| https://w3id.org/linkml/String[`string`]

| Inherited from
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]


|===

////
[.text-left]
--
<<slots_table.description,&#10548;>>
--
////


//[discrete]
[#EnergyConsumers]
=== `EnergyConsumers`
+++Conform loads assigned to this ConformLoadGroup.+++


[cols="h,4",width=65%]
|===
| URI
| `cim:NonConformLoadGroup.EnergyConsumers`
| Cardinality
| 1..*
| Type
| xref::class/NonConformLoad.adoc[`NonConformLoad`]


|===

////
[.text-left]
--
<<slots_table.EnergyConsumers,&#10548;>>
--
////


//[discrete]
[#energyIdentCodeEic]
=== `energyIdentCodeEic`
+++The attribute is used for an exchange of the EIC code (Energy identification Code). The length of the string is 16 characters as defined by the EIC code. For details on EIC scheme please refer to ENTSO-E web site.+++


[cols="h,4",width=65%]
|===
| URI
| http://iec.ch/TC57/CIM100-European#IdentifiedObject.energyIdentCodeEic[`eu:IdentifiedObject.energyIdentCodeEic`]
| Cardinality
| 0..1
| Type
| https://w3id.org/linkml/String[`string`]

| Inherited from
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]


|===

////
[.text-left]
--
<<slots_table.energyIdentCodeEic,&#10548;>>
--
////


//[discrete]
[#mRID]
=== `mRID`
+++Master resource identifier issued by a model authority. The mRID is unique within an exchange context. Global uniqueness is easily achieved by using a UUID, as specified in RFC 4122, for the mRID. The use of UUID is strongly recommended.
For CIMXML data files in RDF syntax conforming to IEC 61970-552, the mRID is mapped to rdf:ID or rdf:about attributes that identify CIM object elements.+++


[cols="h,4",width=65%]
|===
| URI
| `cim:IdentifiedObject.mRID`
| Cardinality
| 1
| Type
| https://w3id.org/linkml/String[`string`]

| Inherited from
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]


|===

////
[.text-left]
--
<<slots_table.mRID,&#10548;>>
--
////


//[discrete]
[#name]
=== `name`
+++The name is any free human readable and possibly non unique text naming the object.+++


[cols="h,4",width=65%]
|===
| URI
| `cim:IdentifiedObject.name`
| Cardinality
| 1
| Type
| https://w3id.org/linkml/String[`string`]

| Inherited from
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]


|===

////
[.text-left]
--
<<slots_table.name,&#10548;>>
--
////


//[discrete]
[#NonConformLoadSchedules]
=== `NonConformLoadSchedules`
+++The NonConformLoadSchedules in the NonConformLoadGroup.+++


[cols="h,4",width=65%]
|===
| URI
| `cim:NonConformLoadGroup.NonConformLoadSchedules`
| Cardinality
| 0..*
| Type
| xref::class/NonConformLoadSchedule.adoc[`NonConformLoadSchedule`]


|===

////
[.text-left]
--
<<slots_table.NonConformLoadSchedules,&#10548;>>
--
////


//[discrete]
[#shortName]
=== `shortName`
+++The attribute is used for an exchange of a human readable short name with length of the string 12 characters maximum.+++


[cols="h,4",width=65%]
|===
| URI
| http://iec.ch/TC57/CIM100-European#IdentifiedObject.shortName[`eu:IdentifiedObject.shortName`]
| Cardinality
| 0..1
| Type
| https://w3id.org/linkml/String[`string`]

| Inherited from
| xref::class/IdentifiedObject.adoc[`IdentifiedObject`]


|===

////
[.text-left]
--
<<slots_table.shortName,&#10548;>>
--
////


//[discrete]
[#SubLoadArea]
=== `SubLoadArea`
+++The SubLoadArea where the Loadgroup belongs.+++


[cols="h,4",width=65%]
|===
| URI
| `cim:LoadGroup.SubLoadArea`
| Cardinality
| 1
| Type
| xref::class/SubLoadArea.adoc[`SubLoadArea`]

| Inherited from
| xref::class/LoadGroup.adoc[`LoadGroup`]


|===

////
[.text-left]
--
<<slots_table.SubLoadArea,&#10548;>>
--
////





== Used by


[cols="1,1",width=65%]
|===
| Source class | Slot name



| xref::class/NonConformLoad.adoc[`NonConformLoad`] | xref::class/NonConformLoad.adoc#LoadGroup[`LoadGroup`]



| xref::class/NonConformLoadSchedule.adoc[`NonConformLoadSchedule`] | xref::class/NonConformLoadSchedule.adoc#NonConformLoadGroup[`NonConformLoadGroup`]


|===

